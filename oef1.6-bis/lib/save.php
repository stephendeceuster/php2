...
function SaveFormData( MessageService $ms, DBManager $dbm, Validator $val, Router $router )
{
    if ( $_SERVER['REQUEST_METHOD'] == "POST" )
    {
        if ( ! isset( $_POST['btnOpslaan'] ) )
        {
            if ( isset($_POST['aftercancel'] )) { $router->GoToPage( $_POST['aftercancel'] ); exit; }
            else { $router->GoHome(); exit; }
        }
